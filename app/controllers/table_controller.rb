class TableController < ApplicationController
    ...
    def team_position       
        ...
        @table_qfinal = Schedule.unscoped.where(id: Schedule.joins(:tour).where("tours.name ~ '1/4'").pluck(:id)).order(id: :asc) if params[:id]=='3' 
        @table_sfinal = Schedule.unscoped.where(id: Schedule.joins(:tour).where("tours.name ~ '1/2'").pluck(:id)).order(id: :asc) if params[:id]=='3' 
        
        @table_third = Schedule.joins(:tour).where("tours.name ~ '3 место'") if params[:id]=='3' 
        @table_final = Schedule.unscoped.where(id: Schedule.joins(:tour).where("lower(tours.name)= 'финал'").pluck(:id)).order(id: :asc) if params[:id]=='3' 
